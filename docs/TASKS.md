...
| B19 | AUDIT residual W11 (cosmetic): mark which build_chain_*.json is current (a latest pointer), so an auditor need not recompute to find the live one | data/graph_dumps | 30 min |
