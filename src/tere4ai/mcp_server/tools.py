...
# DEC-08 scope (decided 2026-07-21 after a live audit; characterization
# corrected 2026-07-30). The ban covers every field TERE4AI composes itself
# (status, answer text, notes, summaries, messages, backlog titles and
# descriptions). The fields below are EXEMPT from that verdict-ban because
# they carry regulatory content, not a TERE4AI verdict. They fall in two
# distinct categories, and only the first is byte-exact:
#
# (a) Byte-exact quotes-of-record: the frozen source span text ("text",
#     "excerpt", "source_text") and verbatim quotes lifted from the frozen
#     corpus or from untrusted project evidence ("source_quote",
#     "target_quote", "quotes"). These are preserved byte-for-byte against
#     the frozen snapshot; that byte-exactness IS the traceability guarantee,
#     so they must never be altered. The EU AI Act's own sentences say things
#     like "compliant with the requirements" (Article 8(2), Article 16 point
#     (a)), and Article 82 is titled "Compliant AI systems which present a
#     risk", so sanitizing such a quote would break traceability.
#
# (b) Normalized deontic extractions of regulatory content ("action",
#     "object", "conditions", "exceptions"): the Institutional-Grammar
#     deontic fields (DEC-03). They carry the regulator's own vocabulary, so
#     they are exempt from the verdict-ban (they are extracted regulatory
#     content, NOT a system verdict), but they are NORMALIZED (case-folding,
#     whitespace normalization, elision of long inline material), so they are
#     NOT claimed to be byte-exact verbatim quotes. The byte-exact
#     quote-of-record for a norm is its source span text (category a),
#     verified char-exact against the snapshot and MANIFEST; the deontic
#     fields are a derived reading of it, served unaltered by the tool but
#     never asserted to be a verbatim quote of the source bytes.
#
# Replayed generator and judge free text ("rationale", "judge_rationale") is
# exempt from this mechanical string scan because it may legitimately quote
# the statute; actual compliance assertions in that text are rejected
# semantically by the runtime grounding judge (Section 7, prompt check "no
# compliance assertion").
#
# NOT exempt: "title". A backlog item's title is MODEL-GENERATED by TERE4AI
# (see backlog.py), so it is system-composed text and must stay inside the
# scan. The only source-text "title" (a Chapter or Article heading) never
# reaches an envelope under a "title" key: source_trace surfaces such a
# heading through the exempt "excerpt"/"text" fields, not as "title".
VERBATIM_QUOTE_FIELDS = frozenset(
    {
        # (a) Byte-exact quotes-of-record: frozen source text, preserved
        #     byte-for-byte.
        "text",
        "excerpt",
        "source_text",
        # (a) Verbatim quotes lifted from the corpus or untrusted evidence.
        "source_quote",
        "target_quote",
        "quotes",
        # (b) Normalized deontic extractions (DEC-03): regulator vocabulary,
        #     exempt as extracted content, NOT asserted byte-exact.
        "action",
        "object",
        "conditions",
        "exceptions",
        # Replayed model free text, judge-gated semantically.
        "rationale",
        "judge_rationale",
    }
)
